...
import { bgColor, commonStyles, h, iphoneX, pt, screenHeight, screenWidth, w } from "../../utils/style";
...
let CustomSafeAreaView = class CustomSafeAreaView extends Component {
  ...
  componentWillMount() {
    ...
    if (iphoneX) {
      this.setState({ paddingHeight: 78 });
    } else {
      this.setState({ paddingHeight: 55 });
    }
  }
  render() {
    ...
    return <View style={styleAssign([w(screenWidth), h(screenHeight), bgColor(commonStyles.whiteColor), pt(notNeedSafe ? 0 : paddingHeight), customStyle])}>
        {children}
